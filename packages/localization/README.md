...
| `defaultTranslations`  | Object    | Default English translations as fallback       |
...
The package exports `defaultTranslations` as a fallback when translations haven't loaded from the API.

```typescript
import { defaultTranslations } from '@cohbrgr/localization';

// Use as fallback
const translations = apiData ?? defaultTranslations;
```
...
const App = () => {
    ...
    useEffect(() => {
        fetch('/api/translation/en')
            .then(res => res.json())
            .then(data => setTranslations(data.keys));
    }, []);

    return (
        <TranslationProvider context={{ lang: 'en', keys: translations }}>
            <HomePage />
        </TranslationProvider>
    );
};
...
const LanguageInfo = () => {
    ...
    return <span>{translate('hero.subtitle')} ({lang})</span>;
};
```
